...
- **Reusable Optimized Image Component (`src/components/OptimizedImage.astro` - Implemented & Refined)**:
  ...
  - **Logic**:
    ...
    - The root `<picture>` tag does _not_ apply a `max-width` style, allowing parent containers to control its rendered size. It applies `aspect-ratio` style if `enforceAspectRatio` is used.
...
- **Photo Linking (Phase 1 - Hash-based Deep Linking):**
  - **Slugs**: Unique, URL-friendly slugs are a **required field** in `image_manifest.json`, pre-generated before build time. Astro pages consume these directly.
  - **Full Gallery Context**: Astro gallery pages (e.g., `/photos/tags/[tag]/[page].astro`) provide the _entire_ set of photos for the current gallery context (e.g., all photos for a specific tag) to the client-side `photoswipe.js` script. This allows PhotoSwipe to open any photo in that context via its slug, regardless of the current thumbnail pagination page.
...
- **Separation of Concerns:**
  - `PhotoGallery.astro`: Handles gallery data (receiving photos with pre-generated slugs), thumbnail layout, and provisioning of all necessary `data-*` attributes for PhotoSwipe core, the caption plugin, and hash-based linking.
...
- **Typography Styling**:
  - A custom Tailwind utility class `.prose` (defined in `src/styles/base.css`) is used to style Markdown-generated content.
...
- **Individual Post Rendering**:
  ...
  - **Layout**: `src/layouts/MarkDownPostLayout.astro` is used. It displays metadata and uses `OptimizedImage.astro` for the featured image. Content is styled with `.prose`. Semantic typography classes are used for layout elements.
- **Listing Pages**:
