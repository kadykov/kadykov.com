# Square Grid Gallery (Contain)

## Description

A basic square grid photo gallery using `object-fit: contain` to show full images without cropping. Photos keep their original aspect ratios, resulting in letterboxing (empty space) around images that don't match the square cells. This approach preserves composition but creates inconsistent gaps that break visual rhythm.
...
photo-gallery, css-grid, object-fit, responsive-design, letterbox
...
/* Square grid with object-fit: contain */
.gallery {
  display: grid;
...
.gallery img {
  width: 100%;
  height: 100%;
  /* Set a minimum height for the cells */
  aspect-ratio: 1 / 1;
  object-fit: contain;
  background: #fff;
